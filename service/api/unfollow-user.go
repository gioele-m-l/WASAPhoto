...
func (rt *_router) unfollowUser(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	if err != nil {
		ctx.Logger.WithError(err).Error("error in unfollowUser function: invalid username")
	...
	if err != nil {
		ctx.Logger.WithError(err).Error("error in unfollowUser function: user not found")
	...
	if userDB.UserID != userTok.UserID {
		ctx.Logger.Error("error in unfollowUser function: forbidden")
	...
	if err != nil {
		ctx.Logger.WithError(err).Error("error in unfollowUser function: error converting string to int")
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	// Check if the userID provided is the same of this user
	if userID2 == userDB.UserID {
		ctx.Logger.Warning("unfollowUser function: user cannot unfollow itself")
		w.WriteHeader(http.StatusForbidden)
		return
	}
	...
	if err != nil {
		ctx.Logger.WithError(err).Error("error in unfollowUser function: specified userID not found")
		w.WriteHeader(http.StatusNotFound)
		return
	}
	...
	if err != nil {
		ctx.Logger.WithError(err).Error("unfollowUser: internal server error")
	...
	err = rt.db.UnfollowUser(userDB.UserID, userDB2.UserID)
	if err != nil {
		ctx.Logger.WithError(err).Warning("unfollowUser: this relationship already does not exist")
